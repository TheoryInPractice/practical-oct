...
python -m experiments.data.generate_synthetic -seeds 15
python -m experiments.data.validate -precomputed synthetic_precomputed_0-14.csv
```
...
These datasets are then sanitized with
```
python -m experiments.data.sanitize
```
...
The `-seeds` argument takes a list of integers to be used as the graph generators' seeds; in our experiments we use 15 seeds.

You can validate that your synthetic graphs have the expected vertex and edge counts by running
```
python -m experiments.data.validate -precomputed [ground_truth.csv]
```
