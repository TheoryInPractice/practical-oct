...
def _percent_valid(df, solver, timeout, cluster):
    ...
    total = len(ratios)
    if not total:
        print(solver, timeout, cluster, "has a total of zero")
        raise ValueError("")
...
if __name__ == "__main__":
    print('Computing quantum heuristic table')
    # Compute table and save as .tex
    compute_approx('results/quantum_heuristic_results.csv',
                   'results/quantum_exact_results.csv',
                   'tables/quantum_heuristic_results.tex')
    print("Wrote quantum_heuristic_results.tex")
    # Save some meta data
    with open('results/quantum_off_by_one.txt', 'w') as outfile:
        for key in sorted(off_by_one.keys()):
            outfile.write('{} : {}\n'.format(str(key), str(off_by_one[key])))
    with open('results/quantum_missing_exact.txt', 'w') as outfile:
